...
def solve_with_lkh(par_file="problem.par", lkh_exec="./LKH"):
...
def solve(
    ...
    if 'TW' in problem_type:
        with open("problem.vrptw", "w") as f:
            f.write(problem)
        par_str = """
        PROBLEM_FILE = problem.vrptw
        OUTPUT_TOUR_FILE = output.tour
        RUNS = 1
        TRACE_LEVEL = 1
        TIME_WINDOW = YES
        """
        with open("/common/home/users/s/scgui/Code/2025-LLMCO/0424-routefinder-LO-CaDA-ReLD-dotNoise/problem.par",
                  "w") as f:
            f.write(par_str)
        action = solve_with_lkh(
            par_file="/common/home/users/s/scgui/Code/2025-LLMCO/0424-routefinder-LO-CaDA-ReLD-dotNoise/problem.par",
            lkh_exec=solver_loc)
        cost = 0
    else:
