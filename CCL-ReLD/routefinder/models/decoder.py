...
class RouteFinderDecoder(AttentionModelDecoder):
    ...
    def forward(
        ...
        logits = self.pointer(torch.stack([glimpse_q, glimpse[-1]]), glimpse_k, glimpse_v, logit_k, mask)

        logits = logits + logdis
...
class ReLDPointerAttention(nn.Module):
    ...
    def forward(self, query, key, value, logit_key, attn_mask=None):
        ...
        q_idt = query[-1]
        query = query[0]
        heads = q_idt + self._inner_mha(query, key, value, attn_mask)
