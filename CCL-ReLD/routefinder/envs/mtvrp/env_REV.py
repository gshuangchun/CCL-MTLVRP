...
class MTVRPEnv(RL4COEnvBase):
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        curr_total_length = (
                td["current_total_length"] + distance * ((curr_node[:, None] != 0) | (td['open_route'] == 0))
        )
        ...
        td.update(
            {
                ...
                "current_total_length": curr_total_length,
        ...
        td_reset = TensorDict(
            {
                ...
                "current_total_length": torch.zeros(
                    (*batch_size, 1), dtype=torch.float32, device=device
                ),  # for distance limits
    ...
    def check_solution_validity(td: TensorDict, actions: torch.Tensor):
        ...
        for ii in range(actions.size(1)):
            ...
            curr_length = torch.round(curr_length * scaling_factor) / scaling_factor
            assert torch.all(
            ...
            curr_time = torch.round(curr_time * scaling_factor) / scaling_factor
            assert torch.all(
        ...
        for ii in range(actions.size(1)):
            ...
            assert (
                    used_cap_l <= td["vehicle_capacity"] + 0.000001
            ).all(), "Used more linehaul than capacity: {} / {}".format(
                used_cap_l, td["vehicle_capacity"]
            )
            assert (
                    used_cap_b <= td["vehicle_capacity"] + 0.000001
    ...
    def load_data(self, fpath, batch_size=[]):
        ...
        td = load_npz_to_tensordict1(fpath)
...
def load_npz_to_tensordict1(filename):
    ...
    x_dict['locs'] = scale(x_dict['locs'], scaling_factor) / scaling_factor
    if 'time_windows' in x_dict.keys():
        x_dict['time_windows'] = scale(x_dict['time_windows'], scaling_factor) / scaling_factor
        x_dict['service_time'] = scale(x_dict['service_time'], scaling_factor) / scaling_factor
    return TensorDict(x_dict, batch_size=batch_size)


def scale(data, scaling_factor: int):
    """
