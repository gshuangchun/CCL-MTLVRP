...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint", type=str,
        default="./logs/0424-routefinder-LO-CaDA-ReLD-dotNoise-P20-newO-100-main/2025-04-27_11-39-29/checkpoints/epoch_299.ckpt",
        help="Path to the model checkpoint"
    )
    parser.add_argument(
        "--problem",
    ...
    if opts.datasets is not None:
        ...
        data_paths = []
        for root, _, files in os.walk("../CCL/data"):
            for file in files:
                # print(file)
                if "test" not in root:
                    continue
    ...
    for dataset in tqdm(data_paths):
        ...
        for batch in dataloader:
            ...
            o = run(policy, td_test, env, device=device)
            res.append(o)
            total_time1 = time.time() - start1
            # print(f"Per Instance Time: {total_time1:.3f} s")
        out = {}
        ...
        gaps.append(results[dataset_name]["gap"])
        objs.append(results[dataset_name]["cost"])
        times.append(inference_time)
    total_params = sum(p.numel() for p in model.policy.encoder.parameters())
    print(f"Enc Parameters: {total_params / 1e6:.2f} M")
    total_params = sum(p.numel() for p in model.policy.decoder.parameters())
    print(f"Dec Parameters: {total_params / 1e6:.2f} M")
    print(f"Peak memory usage: {torch.cuda.max_memory_allocated() / 1e9} GB")
    print(f"Time: {np.array(times).mean():.3f}s")
    print(f"Gap: {np.array(gaps).mean():.3f}s")
    print(f"Obj: {np.array(objs).mean():.3f}s")
